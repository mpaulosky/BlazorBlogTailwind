namespace BlazorBlog.Web.Playwright.Tests;
...
public class PlaywrightWebApplicationFactory : WebApplicationFactory<Web.Program>, IAsyncLifetime
{
	...
	public async Task<IPage> CreatePlaywrightPageAsync(bool headless = true)
	{
		...
		return await browser.NewPageAsync(new BrowserNewPageOptions() { BaseURL = uri });
	}
...
#pragma warning disable CS8774 // Member must have a non-null value when exiting.
		...
		browser ??= (await playwright.Chromium.LaunchAsync(new() { Headless = _Headless, Devtools = true })) ??
